...
def test_vnext_no_callbacks_listen_to_ticker_value():
...
def test_vnext_store_market_only_updates_on_refresh_button():
    if not getattr(dash_app, "DASH_AVAILABLE", False):
        pytest.skip("Dash not available")
    app = dash_app.app
...
def test_vnext_store_analysis_key_only_updates_on_run_analysis_button():
    if not getattr(dash_app, "DASH_AVAILABLE", False):
        pytest.skip("Dash not available")
    app = dash_app.app
    metas = _find_callback_metas_for_output(app.callback_map, "store-analysis-key.data")
    assert len(metas) == 1, "Expected exactly one callback for store-analysis-key.data"
    assert _inputs_set(metas[0]) == {"run-analysis-button.n_clicks"}


def test_vnext_store_ref_only_updates_on_ticker_submit_or_blur():
    if not getattr(dash_app, "DASH_AVAILABLE", False):
        pytest.skip("Dash not available")
    app = dash_app.app
